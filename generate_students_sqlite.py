"""
generate_students.py  (SQLite version)
------------------------------------------------------------------
Same persona-weighted generation logic as before. Two changes from
the original:
  1. Reads catalog CSVs from ./student_data/ instead of the same
     folder as this script.
  2. Writes to the local SQLite app_data.db (via db.py) instead of
     MongoDB -- no internet connection required.
...
def main():
    ...
    conn = db.get_client()
    db.init_db(conn)
    conn.execute("DELETE FROM students")  # clean slate on each run
    for s in students:
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (
                s["student_id"], s["name"], s["email"], s["degree_id"], s["degree_name"],
                s["track_id"], s["track_name"], s["year_standing"], s["persona"],
                json.dumps(s["completed_courses"]), json.dumps(s["grades"]),
            )
        )
    conn.commit()
    print(f"Inserted {len(students)} students into {db.DB_PATH}")
    conn.close()
